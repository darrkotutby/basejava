...
- Рефакторинг: в конструктор by.tut.darrko.webapp.model.Resume добавить второй параметр `fullName`
- Сделать рефакторинг всех реализаций `Storage`: заменить метод `by.tut.darrko.webapp.model.Resume[] getAll()` на `List<by.tut.darrko.webapp.model.Resume> getAllSorted()`
